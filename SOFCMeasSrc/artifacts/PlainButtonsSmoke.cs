    ...
    [STAThread] static void Main() { try { Run(); } catch (Exception ex) { Console.WriteLine(ex.GetType().FullName + ": " + ex.Message); if (ex.InnerException != null) Console.WriteLine(ex.InnerException.Message); Environment.ExitCode = 1; } } static void Run()
    {
        ...
        using (var form = new MainForm())
        {
            ...
            using (var auth = new AuthorityDialog(false))
            {
                ...
                var role = (RadioButton)auth.Controls["rdoAdministrator"];
                ...
                role.Checked = true; password.Text = "wrong"; apply.PerformClick();
            ...
            var minimizeButton = (Button)Field(form, "btnMinimize");
            for (int repeat = 0; repeat < 3; repeat++)
            {
                minimizeButton.PerformClick(); Application.DoEvents();
                ...
                if (minimizeButton.BackColor != Color.FromArgb(54,65,82) || minimizeButton.Image == null || minimizeButton.UseVisualStyleBackColor) throw new Exception("Button style changed after restore");
            }
