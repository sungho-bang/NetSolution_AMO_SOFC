...
static void Main(string[] args){
 ...
 Check(get("ApplicationRootDirectory")==root,"Root");Check(get("ConfigurationFilePath")==Path.Combine(root,"conf","SOFCMeas.config"),"conf");Check(get("LogRootDirectory")==Path.Combine(root,"log"),"log");Check(get("DataRootDirectory")==Path.Combine(root,"Data"),"Data");
 if(args.Length==0){Console.WriteLine("PASS: deployed default paths C:\\SOFCMeas bin/conf/log/Data");return;}
 ...
 var readback=(ICollection)storage.GetType().GetMethod("ReadSamples",I).Invoke(storage,new object[]{record});Check(readback.Count==1,"Review samples");Console.WriteLine("PASS: review reading, log writing, LOT CSV output, saved-result index/query using shared root");
}}
