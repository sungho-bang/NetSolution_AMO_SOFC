...
var started=date.AddHours(9+station);string lotNo="LOT"+date.ToString("dd")+"01";
...
for(int j=0;j<60;j++){
double t=j/59D; double shape=station==1 ? (t<=0.86 ? Smooth(t/0.86) : 1-0.08*Smooth((t-0.86)/0.14)) : (t<=0.67 ? Smooth(t/0.67) : 1-0.9*Smooth((t-0.67)/0.33));int raw=(int)Math.Round(1000*(0.02+(peak-0.02)*shape+(rng.NextDouble()-0.5)*0.0006));raw=Math.Max(20,raw);double kgf=raw/1000D;max=Math.Max(max,kgf);
...
Set(lot,"Inspections",inspections);string path=(string)storage.GetType().GetMethod("SaveLot",I).Invoke(storage,new object[]{lot});if(!path.Contains("PLC"+station+Path.DirectorySeparatorChar))throw new Exception("PLC folder");var lines=File.ReadAllLines(path);if(lines.Length!=15||!lines[0].StartsWith("모델명,")||!lines[2].StartsWith("NO.,판정결과,"))throw new Exception("CSV layout");
