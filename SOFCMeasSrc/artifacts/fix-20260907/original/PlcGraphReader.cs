...
namespace SOFCMeas
{
    ...
    internal sealed class PlcGraphReader : IDisposable
    {
        ...
        private readonly PlcGraphSettings m_Settings;
        private readonly PlcGraphProtocol m_Protocol = new PlcGraphProtocol();
        private readonly DeviceAddress m_RequestDevice;
        private readonly DeviceAddress m_LoadRawDevice;
            ...
            int collectionVersion)
        {
            ...
            if (readLoadRaw)
            {
                await client.OptimizedReadAsync(builder =>
                {
                    // Int32 reads two consecutive WORDs in one McpX value:
                    // D801 = low 16 bits, D802 = high 16 bits (signed two's complement).
                    builder.Add<int>(
                        m_LoadRawDevice.Prefix,
                        m_LoadRawDevice.Address,
                        value => snapshot.LoadRaw = value);
                }).ConfigureAwait(false);
            }
        ...
        private void ApplyUpdate(PlcGraphUpdate update)
        {
            ...
            if (update.AddSample)
            {
                // Scale is a divisor, e.g. raw 500 / 1000 = 0.500 kgf.
                double loadKgf = update.LoadRaw / m_Settings.LoadScale;
                ...
                RaiseLogMessage(
                    ...
                    string.Format(
                        CultureInfo.InvariantCulture,
                        "cycle={0} sample={1} raw={2} loadKgf={3:0.000000} readTime={4:O}",
                        m_CycleNumber,
                        sampleNumber,
                        update.LoadRaw,
                        loadKgf,
