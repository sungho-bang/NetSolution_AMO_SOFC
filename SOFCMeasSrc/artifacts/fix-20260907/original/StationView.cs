...
namespace SOFCMeas
{
    public partial class StationView : UserControl
    {
        ...
        private string m_LoadDeviceText = "D801";
        private int? m_LastLoadRaw;
        ...
        private void ShowSelectedReview()
        {
            ...
            if (samples.Count > 0)
            {
                ...
                marker.Points[0].Label = "최대 " + peak.LoadKgf.ToString("0.000", CultureInfo.InvariantCulture) + " kgf";
        ...
        private void UpdateReviewSummary()
        {
            ...
            int ng = m_ReviewInspections.Count(i => i.Record.Result == "NG" || i.Record.Result == "FAIL");
        ...
        internal void ConfigureGraph(int station)
        {
            ...
            // The empty/startup graph must use the inspection SPEC as its
            // visible Y-axis basis. Auto mode adds the configured 10% margin.
            m_YMax = Math.Max(0.001D, specMaximum);
            ...
            txtSpec.Text = specMaximum.ToString("0.00", CultureInfo.InvariantCulture);
            ResetLoadGraph();
        }
        ...
        public StationView()
        {
            ...
            cboInspectionNumber.SelectedIndexChanged += delegate { UpdateReviewSummary(); };
        ...
        private async System.Threading.Tasks.Task RunSimulationAsync(int intervalMilliseconds = 300)
        {
            ...
            catch (OperationCanceledException) { if (!IsDisposed) { ResetLoadGraph(); m_LastLoadRaw = null; m_LastLoadKgf = 0; UpdateCurrentLoadText(); AppendLog(DateTime.Now, "SIMULATION STOP: 진행 중 데이터 초기화 / 완료 검사 " + m_InspectionCount + "회 유지"); } }
        ...
        private static void MatchInputLabel(Label label, TextBox input)
        {
        ...
        {
            get { return txtSpec.Text; }
            set { txtSpec.Text = value; }
        }
        ...
        internal StationInspectionResult CompleteInspection()
        {
            ...
            double lowerSpec;
            double upperSpec;
            if (!TryParseSpecRange(txtSpec.Text, out lowerSpec, out upperSpec))
            {
                ApplyVerdict("INVALID", InvalidColor, InvalidBackColor);
                return CreateInvalidResult(
                    "SPEC 기준값이 올바르지 않습니다. 예: 0.40");
            }
        ...
        private void ShowLivePeakMarker()
        {
            ...
            marker.Points[0].Label = "최대 " +
                m_PeakLoadKgf.ToString("0.000", CultureInfo.InvariantCulture) +
                " kgf";
        ...
        {
            // Display the original unsigned WORD bit patterns, including negative Int32 loads.
            string low = m_LastLoadRaw.HasValue
                ? unchecked((ushort)m_LastLoadRaw.Value).ToString(CultureInfo.InvariantCulture) : "0000";
            string high = m_LastLoadRaw.HasValue
                ? unchecked((ushort)(m_LastLoadRaw.Value >> 16)).ToString(CultureInfo.InvariantCulture) : "0000";
            lblCurrentLoad.Text = string.Format(CultureInfo.InvariantCulture,
                "{0}[{1}][{2}]\r\n{3:0.000} kgf  |  검사 횟수 {4}",
                m_LoadDeviceText, low, high, m_LastLoadKgf, m_InspectionCount);
        }

        internal void SetJobInformationEditing(bool editing)
        {
            m_JobInformationEditing = editing;
            txtModel.ReadOnly = !editing;
            txtOperator.ReadOnly = !editing;
            txtLotNumber.ReadOnly = !editing;
