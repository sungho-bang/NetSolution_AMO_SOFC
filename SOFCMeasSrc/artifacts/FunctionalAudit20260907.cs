...
internal static class FunctionalAudit20260907
{
    static Assembly App;
    ...
    static object Call(object o, string name, params object[] args)
    {
        ...
        try { return t.GetMethod(name, Flags).Invoke(o is Type ? null : o, args); }
    ...
    static void Tests()
    {
        ...
        {
            Finding((bool)Get(Get(view,"txtLotNumber"),"ReadOnly") && (bool)Get(Get(view,"txtOperator"),"ReadOnly"),"initial LOT/model/operator fields are read-only with no wired editing action");
            ...
            Finding(!(bool)Get(result,"IsPass"), "SPEC=0.405 peak=0.407 shouldGOOD but actual=" + ((bool)Get(result,"IsPass")?"GOOD":"NG") + " threshold="+Get(result,"LowerSpecKgf"));
            cfg["Station1.Inspection.ThresholdKgf"]="0.40"; cfg["Station1.Graph.YMax"]="2"; cfg["Station1.Graph.YAuto"]="false"; Save(cfg);
            Call(view,"ConfigureGraph",1);
            Finding(Math.Abs(chart.ChartAreas[0].AxisY.Maximum-2)>.0001, "YMax=2 YAuto=false actual="+chart.ChartAreas[0].AxisY.Maximum);
            ...
            Finding(axisError || chart.ChartAreas[0].AxisY.Maximum <= chart.ChartAreas[0].AxisY.Minimum, "validated YMin=1 YMax=2 leads invalid chart axis");
            ...
            Finding(accepted,"configuration accepts equal START and END values");
            ...
            Finding((int)Get(plcSettings,"Port")!=12345,"validated Port=12345.0 reloads as "+Get(plcSettings,"Port"));
        ...
        using(var view=(Control)New("StationView"))
        {
            ...
            Finding(ReferenceEquals(review,Get(view,"m_ReviewChart")) && review.Series[0].Points.Count==3 && (bool)Get(view,"m_ShowingReviewSummary"),"select inspection2 updates NG summary while graph remains inspection1 until REVIEW");
        ...
        using(var locked=new FileStream(index,FileMode.Open,FileAccess.Read,FileShare.None))
            try {Call(storage,"SaveLot",lot);} catch(IOException){threw=true;}
        int after=Directory.GetFiles(data,"*.csv",SearchOption.AllDirectories).Length;
        Call(storage,"SaveLot",lot);
        Finding(threw && after==before+1 && Directory.GetFiles(data,"*.csv",SearchOption.AllDirectories).Length==before+2,"index lock reports savefailure afterCSV committed; retry duplicates sameLOT");
        ...
        using(var form=(Form)New("SOFCMeas"))
        {
            ...
            Finding(Get(reader,"m_WorkerTask")==null,"manualSTART cannot connect when AutoConnect=false");
            ...
            Finding((int)Get(view,"m_InspectionCount")==3 && (int)Get(view,"m_PassCount")==1,"rolling list=2NG but live summary=3,totalGOOD1");
            ...
            Finding((bool)Get(Get(view,"txtLotNumber"),"ReadOnly") && (bool)Get(Get(view,"txtModel"),"ReadOnly"),"LOT end leaves model and LOT fields read-only");
    ...
    static void PolicyTests()
    {
        ...
        Finding(Get(badLog,"LastError")!=null,"runtime logwrite failure is swallowed and only stored in LastError (only bootstrap observes it)");
    }
    static void NetworkTests()
    {
        using(var server1=new MockPlc()) using(var server2=new MockPlc())
        {
            ...
            for(int i=0;i<2;i++)
            {
                ...
                Set(settings,"AutoConnect",true); Set(settings,"PollIntervalMilliseconds",100); Set(settings,"TimeoutMilliseconds",500);
