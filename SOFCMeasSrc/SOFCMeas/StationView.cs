...
namespace SOFCMeas
{
    public partial class StationView : UserControl
    {
        ...
        internal void ConfigureGraph(int station, bool resetGraph = true)
        {
            ...
            if (resetGraph) ResetLoadGraph();
            else ApplyGraphConfigurationToCurrentData();
        }
        ...
        internal void ApplyCollectionEnabled(bool enabled)
        {
            if (CollectionEnabled != enabled) SetCollectionState(enabled);
        }

        private async System.Threading.Tasks.Task RunSimulationAsync(int intervalMilliseconds = 0)
        {
            ...
            try
            {
                ...
                if (intervalMilliseconds <= 0)
                    intervalMilliseconds = (int)SettingsCatalog.Number(
                        settings,
                        stationKey + "Plc.PollIntervalMs");
                ...
                StateText = "SIMULATION"; StateColor = Color.DarkOrange;
                    ...
                    AppendLog(
                        ...
                        string.Format(
                            ...
                            "SIMULATION 시작: {0}ms 간격, 60회 가상 하중 수집",
        ...
        internal void ApplyInspectionThreshold(double threshold)
        {
            SetInspectionThreshold(threshold);
        }
        ...
        internal void ResetLoadGraph()
        {
            ...
            if (!m_AutoX) ApplyXAxisInterval(chartArea, m_XInterval);
            if (!m_AutoY) ApplyYAxisInterval(chartArea, m_YInterval);
        ...
        private void ApplyGraphConfigurationToCurrentData()
        {
            Series series = chartLoad.Series[0];
            while (m_MaximumGraphPoints > 0 &&
                   series.Points.Count > m_MaximumGraphPoints)
            ...
            ChartArea chartArea = chartLoad.ChartAreas[0];
            chartArea.AxisX.Minimum = m_XMin;
            chartArea.AxisX.Maximum = m_AutoX
                ? m_XMin + (Math.Max(m_XMax, m_GraphReadCount) - m_XMin) * 1.1D
                : m_XMax;
            chartArea.AxisX.Interval = m_AutoX
                ? (chartArea.AxisX.Maximum - chartArea.AxisX.Minimum) / 10D
                : m_XInterval;

            double dataMinimum = m_YMin;
            if (m_HasLoadSample && series.Points.Count > 0)
                dataMinimum = Math.Min(dataMinimum, series.Points.Min(point => point.YValues[0]));
            chartArea.AxisY.Minimum = m_AutoY
                ? Math.Min(m_YMin, Math.Min(0D, dataMinimum * 1.1D))
                : m_YMin;
            double dataMaximum = m_HasLoadSample ? m_PeakLoadKgf : m_YMax;
            double configuredMaximum = Math.Max(m_YMax, dataMaximum);
            chartArea.AxisY.Maximum = m_AutoY
                ? configuredMaximum + (configuredMaximum - chartArea.AxisY.Minimum) * 0.1D
                : m_YMax;
            chartArea.AxisY.Interval = m_AutoY
                ? (chartArea.AxisY.Maximum - chartArea.AxisY.Minimum) / 10D
                : m_YInterval;
        ...
        private static void ApplyXAxisInterval(ChartArea area, double interval)
        {
            area.AxisX.Interval = interval;
            area.AxisX.MajorGrid.Interval = interval;
            area.AxisX.MajorTickMark.Interval = interval;
            area.AxisX.LabelStyle.Interval = interval;
        }

        private static void ApplyYAxisInterval(ChartArea area, double interval)
        {
            area.AxisY.Interval = interval;
            area.AxisY.MajorGrid.Interval = interval;
            area.AxisY.MajorTickMark.Interval = interval;
            area.AxisY.LabelStyle.Interval = interval;
        }
