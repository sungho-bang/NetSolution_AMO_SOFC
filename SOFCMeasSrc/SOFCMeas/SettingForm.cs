...
namespace SOFCMeas
{
    public class SettingForm : Form
    {
        ...
        internal Func<string> ValidateRuntimeApply = () => null;
        internal Func<Task> ApplySavedSettingsAsync = () => Task.FromResult(0);
        ...
        public SettingForm()
        {
            ...
            footer.Controls.Add(new Label { Dock = DockStyle.Fill, Text = "SAVE 후 새 설정은 다음 측정부터 적용됩니다. 측정·LOT 저장 중에는 SAVE할 수 없습니다. X: 샘플 횟수 / Y: 하중(kgf).", TextAlign = ContentAlignment.MiddleLeft });
            ...
            save.Click += async delegate
            {
                ...
                try
                {
                    await SaveValuesAndApplyAsync();
                    MessageBox.Show(
                        this,
                        "설정을 저장하고 실행 중인 측정 설정에 적용했습니다.\r\n변경한 읽기 주기는 다음 PLC 읽기부터 사용됩니다.",
                        "저장 완료",
                ...
                {
                    MessageBox.Show(this, ex.Message, "저장/적용 실패", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                finally
                {
                    if (!IsDisposed) save.Enabled = true;
                }
            };
            ResumeLayout(true);
        }
        internal async Task SaveValuesAndApplyAsync()
        {
            if (!CanSave()) throw new UnauthorizedAccessException("ADMINISTRATOR 권한이 필요합니다.");
            string runtimeError = ValidateRuntimeApply == null
                ? null
                : ValidateRuntimeApply();
            if (!string.IsNullOrWhiteSpace(runtimeError))
                throw new InvalidOperationException(runtimeError);

            SaveValues();
            Func<Task> apply = ApplySavedSettingsAsync;
