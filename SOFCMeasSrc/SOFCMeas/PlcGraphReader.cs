    ...
    internal sealed class PlcGraphReader : IDisposable
    {
        ...
        internal void SetCollectionEnabled(bool enabled)
        {
            lock (m_SyncRoot)
            {
                ...
                m_CycleActive = false;
                m_PostStopEndsAt = DateTime.MinValue;
            }
        }
        ...
        private DateTime m_PostStopEndsAt;
        private bool m_HasLoggedRequestValue;
        private ushort m_LastLoggedRequestValue;
            ...
            bool startReader)
        {
            ...
            lock (m_SyncRoot)
            {
                ...
                m_PostStopEndsAt = DateTime.MinValue;
                bool requestSourceChanged =
                    !string.Equals(
                        previousSettings.IpAddress,
                        settings.IpAddress,
                        StringComparison.OrdinalIgnoreCase) ||
                    previousSettings.Port != settings.Port ||
                    !string.Equals(
                        previousSettings.RequestDevice,
                        settings.RequestDevice,
                        StringComparison.OrdinalIgnoreCase) ||
                    previousSettings.StartRequestValue != settings.StartRequestValue ||
                    previousSettings.EndRequestValue != settings.EndRequestValue;
                if (requestSourceChanged) m_HasLoggedRequestValue = false;
            }
            ...
            RaiseLogMessage(
                ...
                string.Format(
                    CultureInfo.InvariantCulture,
                    "endpoint={0}:{1} pollMs={2} postStopReadMs={3} reconnectMs={4} timeoutMs={5} " +
                    "requestDevice={6} loadRaw={7}",
                    settings.IpAddress,
                    settings.Port,
                    settings.PollIntervalMilliseconds,
                    settings.PostStopReadMilliseconds,
        ...
        private async Task MonitorAsync(McpX client, CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                lock (m_SyncRoot)
                {
                    ...
                    {
                        m_LastIgnoredStartState = null;
                        if (acquisitionState == PlcAcquisitionState.PostStopCollecting)
                        {
                            update = CreatePostStopUpdate(
                                snapshot,
                                snapshot.ReadTime >= m_PostStopEndsAt);
                        }
                        else
                        {
                            ...
                            if (update.ResetGraph)
                            {
                                m_AcquisitionState = PlcAcquisitionState.Collecting;
                                     ...
                                     m_Settings.PostStopReadMilliseconds > 0)
                            {
                                m_PostStopEndsAt = snapshot.ReadTime.AddMilliseconds(
                                    m_Settings.PostStopReadMilliseconds);
                                m_AcquisitionState = PlcAcquisitionState.PostStopCollecting;
                                update.FreezeGraph = false;
                                update.AddSample = !snapshot.InvalidLoadValue &&
                                                   snapshot.LoadKgf.HasValue;
                                update.IsCollecting = true;
                                RaiseLogMessage(
                                    PlcGraphLogLevel.Info,
                                    "POST_STOP_COLLECTION_STARTED",
                                    string.Format(
                                        CultureInfo.InvariantCulture,
                                        "cycle={0} durationMs={1} requestDevice={2} loadRaw={3}",
                                        m_CycleNumber,
                                        m_Settings.PostStopReadMilliseconds,
                ...
                int pollIntervalMilliseconds = m_Settings.PollIntervalMilliseconds;
                lock (m_SyncRoot)
                {
                    if (m_AcquisitionState == PlcAcquisitionState.PostStopCollecting)
                    {
                        int untilPostStopEnd = checked((int)Math.Min(
                            int.MaxValue,
                            Math.Max(
                                0D,
                                Math.Ceiling((m_PostStopEndsAt - DateTime.Now).TotalMilliseconds))));
            ...
            int collectionVersion)
        {
            ...
            lock (m_SyncRoot)
            {
                PlcAcquisitionState state = m_AcquisitionState;
                bool startValueActive =
                    (state == PlcAcquisitionState.WaitStart ||
                     state == PlcAcquisitionState.Collecting) &&
                    snapshot.RequestValue == m_Settings.StartRequestValue;
                bool endValueJustReceived =
                    state == PlcAcquisitionState.Collecting &&
                    snapshot.RequestValue == m_Settings.EndRequestValue;
                readLoadRaw = collectionVersion == m_CollectionVersion &&
                    m_CollectionEnabled &&
                    (startValueActive || endValueJustReceived ||
                     state == PlcAcquisitionState.PostStopCollecting);
            }
        ...
        private void ApplyUpdate(PlcGraphUpdate update)
        {
            if (update.RequestChanged &&
                (!m_HasLoggedRequestValue ||
                 update.RequestValue != m_LastLoggedRequestValue))
            {
                m_HasLoggedRequestValue = true;
                m_LastLoggedRequestValue = update.RequestValue;
        ...
        private PlcGraphUpdate CreatePostStopUpdate(
            PlcGraphSnapshot snapshot,
            bool freezeGraph)
        {
            return new PlcGraphUpdate
            {
                RequestChanged = true,
                InvalidRequestValue =
                    snapshot.RequestValue != m_Settings.StartRequestValue &&
                ...
                AddSample = !snapshot.InvalidLoadValue && snapshot.LoadKgf.HasValue,
                FreezeGraph = freezeGraph,
                IsCollecting = !freezeGraph
            };
        }
